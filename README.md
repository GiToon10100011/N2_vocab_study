...
| 경로 | 하는 일 |
|---|---|
| `/` | 오늘 볼 것(신규 / 복습 / 취약) + 학습 시작. Enter 로도 시작된다 |
| `/study` | 학습 세션. 키보드만으로 끝난다 |
| `/add` | 단어 추가. Enter 로 칸 이동 → 저장 → 첫 칸 복귀 |
...
```bash
npm run dev          # 개발 서버
npm test             # 순수 로직 테스트 27개 (SRS 전이표·유형 분포·큐·카드 누출)
npm run test:db      # 실제 Neon DB 왕복 테스트 7개 (테스트 행은 자동으로 지운다)
