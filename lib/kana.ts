...
/*
 * IME 조합에서 읽기를 추정하는 기능은 만들지 않는다.
 * macOS 일본어 IME 의 라이브 변환이 켜져 있으면 타이핑 도중 실시간으로 한자가 만들어져
 * 완성된 가나(ふきゅう)가 조합 문자열에 한 번도 나타나지 않는다.
 * compositionupdate 의 data 를 보든 입력값을 보든 마지막 가나는 잡을 수 없고,
 * 결과는 항상 한 글자 모자란 ふきゅ 가 된다. 시스템 설정에 의존하는 기능은 만들지 않는다.
 * 읽기는 이 칸에 로마자로 직접 친다.
 */
